...
class FavoriteRepository @Inject constructor(@ApplicationContext private val context: Context) : IFavoriteRepository {
    ...
    override fun getAllFavUser(): Flow<MutableList<GithubUser.Item>> {
        return mFavDao.loadAll().map { entities ->
            entities.map { entity ->
                DataMapper.mapEntityItemToDomain(entity)
            }.toMutableList()
        }
    }


    override suspend fun insert(fav: GithubUser.Item) {
        val entity = DataMapper.mapDomainToEntity(fav)
        withContext(Dispatchers.IO) {
            mFavDao.insertFav(entity)
        }
    }

    override suspend fun delete(fav: GithubUser.Item) {
        val entity = DataMapper.mapDomainToEntity(fav)
        withContext(Dispatchers.IO) {
            mFavDao.deleteFav(entity)
        }
    }
